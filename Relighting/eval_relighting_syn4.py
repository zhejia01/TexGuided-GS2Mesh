...
if __name__ == '__main__':
    ...
    for task_name in task_names:
        ...
        ### final hyper
        if "/air_baloons/" in args.gs_input_path:
            base_color_scale = torch.tensor([1.3746, 0.6428, 0.7279], dtype=torch.float32)
        elif "/chair/" in args.gs_input_path:
            base_color_scale = torch.tensor([1.8865, 1.9675, 1.7410], dtype=torch.float32)
        elif "/hotdog/" in args.gs_input_path:
            base_color_scale = torch.tensor([2.0216, 1.6514, 1.0316], dtype=torch.float32)
        elif "/jugs/" in args.gs_input_path:
            # gaussians.base_color_scale = torch.tensor([1.1916, 0.9296, 0.5684], dtype=torch.float32, device="cuda")
            base_color_scale = torch.tensor([0.3874, 0.3093, 0.1765], dtype=torch.float32)
        else:
            raise NotImplementedError
        psnr_pbr = 0.0
        ssim_pbr = 0.0
