...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, pbr_kwargs=None):
    render_path = os.path.join(model_path, name, "ours_deform{}".format(iteration), "renders")
    gts_path = os.path.join(model_path, name, "ours_deform{}".format(iteration), "gt")
    normal_path = os.path.join(model_path, name, "ours_deform{}".format(iteration), "normal")
    makedirs(render_path, exist_ok=True)
    makedirs(gts_path, exist_ok=True)
    makedirs(normal_path, exist_ok=True)
    if gaussians.use_pbr:
        base_color_path = os.path.join(model_path, name, "ours_deform{}".format(iteration), "base_color")
        roughness_path = os.path.join(model_path, name, "ours_deform{}".format(iteration), "roughness")
        lights_path = os.path.join(model_path, name, "ours_deform{}".format(iteration), "lights")
        local_lights_path = os.path.join(model_path, name, "ours_deform{}".format(iteration), "local_lights")
        global_lights_path = os.path.join(model_path, name, "ours_deform{}".format(iteration), "global_lights")
        visibility_path = os.path.join(model_path, name, "ours_deform{}".format(iteration), "visibility")
...
def render_sets(dataset : ModelParams, pipeline : PipelineParams, skip_train : bool, skip_test : bool):
    with torch.no_grad():
        ...
        if args.checkpoint:
            ...
            # gaussians.load_ply(os.path.join(args.checkpoint))
        ...
        if iteration is not None and gaussians.use_pbr:
            ...
            if args.checkpoint:
                env_checkpoint = os.path.dirname(args.checkpoint) + "/env_light_" + "chkpnt" + str(iteration) + ".pth"
