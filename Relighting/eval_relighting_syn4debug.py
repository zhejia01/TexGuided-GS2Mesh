...
from scene.utils import load_img_rgb, srgb_to_rgb
import warnings
...
if __name__ == '__main__':
    ...
    for task_name in task_names:
        ...
        #     gaussians.base_color_scale = torch.tensor([1.0459, 0.0908, 0.8382], dtype=torch.float32, device="cuda")
        # elif "/chair/" in args.model_path:
        #     gaussians.base_color_scale = torch.tensor([1.4176, 1.4093, 1.2635], dtype=torch.float32, device="cuda")
        # elif "/hotdog/" in args.model_path:
        #     gaussians.base_color_scale = torch.tensor([1.3688, 1.2565, 1.0150], dtype=torch.float32, device="cuda")
        # elif "/jugs/" in args.model_path:
        #     # gaussians.base_color_scale = torch.tensor([1.1916, 0.9296, 0.5684], dtype=torch.float32, device="cuda")
        #     gaussians.base_color_scale = torch.tensor([0.6358, 0.5627, 0.4161], dtype=torch.float32, device="cuda")
        # else:
        #     raise NotImplementedError
        psnr_pbr = 0.0
        ssim_pbr = 0.0
        ...
        albedo_scale = torch.tensor([0, 0, 0], dtype=torch.float32, device="cuda")
        for idx, frame in enumerate(tqdm(frames, leave=False)):
            ...
            # if idx == 0:
            albedo_scale += (srgb_to_rgb(gt_albedo) / srgb_to_rgb(render_pkg['base_color'].clamp(1e-6, 1)))[:, mask[0] > 0].median(dim=1).values
            # print("Albedo scale:", albedo_scale)
        albedo_scale /= len(frames)
        print("Albedo scale:", albedo_scale)
        # psnr_pbr /= len(frames)
        # ssim_pbr /= len(frames)
